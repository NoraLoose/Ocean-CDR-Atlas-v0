...
def add_additional_coords(ds: xr.Dataset, df:pd.DataFrame):
    case = ds.title
...
def open_compress_and_save_file(filepath: str | pathlib.Path, out_path_prefix: str | pathlib.Path, df:pd.DataFrame) -> None:
    ds = (xr.open_dataset(filepath, engine='netcdf4')
          .pipe(set_coords)
          .pipe(add_additional_coords, df)
...
def glob_nc_files(base_path: str | pathlib.Path, case:str, done_cases:list[str]):
...
@memory.cache
def process_case(case: str, base_path: str, out_path_prefix: str, done_cases:list[str], df:pd.DataFrame):
    nc_files = glob_nc_files(base_path=base_path, case=case, done_cases=done_cases)
    tasks = []
    for file in nc_files:
        tasks.append(dask.delayed(open_compress_and_save_file)(file, out_path_prefix=out_path_prefix, df=df))
